...
def potential_evolution(wfn_1_old, wfn_2_old, time_step, inter_int, intra_int):
    wfn_1 = wfn_1_old * cp.exp(-1j * time_step * (intra_int * cp.abs(wfn_1_old) ** 2 + inter_int * cp.abs(wfn_2_old) ** 2))
    wfn_2 = wfn_2_old * cp.exp(-1j * time_step * (intra_int * cp.abs(wfn_2_old) ** 2 + inter_int * cp.abs(wfn_1_old) ** 2))
