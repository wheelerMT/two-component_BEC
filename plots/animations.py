...
# ------------------------------------------------------------------------------------------------------------------
fig, ax = plt.subplots(2, 2, figsize=(6.4*2, 9.6), dpi=150)
...
# Initial frame plot:
densPlus_plot = ax[0, 0].pcolormesh(X, Y, abs(psi_1[:, :, 0]) ** 2, vmin=0, vmax=4500, shading='auto', cmap='gnuplot')
densMinus_plot = ax[0, 1].pcolormesh(X, Y, abs(psi_2[:, :, 0]) ** 2, vmin=0, vmax=4500, shading='auto', cmap='gnuplot')
...
pseudoVortPlus_plot = ax[1, 0].pcolormesh(X, Y, pseudo_vort_1.real, vmin=-300, vmax=300, shading='auto', cmap='jet')
pseudoVortMinus_plot = ax[1, 1].pcolormesh(X, Y, pseudo_vort_2.real, vmin=-300, vmax=300, shading='auto', cmap='jet')
...
def animate(i):
    ...
    densMinus_plot.set_array((abs(psi_2[:, :, i]) ** 2).ravel())
